...
const TEXTS=['all serviced — tap the case, or ✓ done, to slide it home','circle a finger on the brass screw (or tap it) to change the flint','circle a finger on the screw (or tap it) to seat the spring back down','tank’s full — tap the felt pad to fold it shut'];
for (const [n,w,h] of [['se',320,568],['ip',390,844],['ipmax',430,932]]) {
  ...
  const r = await p.evaluate((texts)=>{
    ...
    for (const t of texts){
      el.textContent=t; el.classList.add('wrap','show');
      const hr=el.getBoundingClientRect(), dr=d.getBoundingClientRect();
      out.push({t:t.slice(0,28), lines:Math.round(hr.height/20), hintTop:Math.round(hr.top), doneBottom:Math.round(dr.bottom), overlap:hr.top<dr.bottom});
    }
    return out;
  },TEXTS);
  console.log('=== '+n+' '+w+'x'+h);
  for(const x of r) console.log((x.overlap?'OVERLAP ':'   ok   ')+JSON.stringify(x));
  await p.close();
}
await browser.close();
